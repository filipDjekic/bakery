...
test('customer completes catalog, cart, checkout and confirmation flow', async ({ page, seed }) => {
  await page.goto('/proizvodi');
  await page.getByRole('button', { name: `Dodaj ${seed.availableProductName} u korpu` }).click();
  ...
  const date = DateTime.now().setZone('Europe/Belgrade').plus({ days: 1 }).toISODate();
...
test('unavailable product cannot be added', async ({ page, seed }) => {
  await page.goto('/proizvodi');
  await expect(page.getByRole('button', { name: `${seed.unavailableProductName} je rasprodat` })).toBeDisabled();
});
