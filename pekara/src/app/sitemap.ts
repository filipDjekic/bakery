...
export function buildSitemap(baseUrl: URL, products: SitemapProduct[]): MetadataRoute.Sitemap {
  return [
    { url: new URL('/', baseUrl).href, changeFrequency: 'daily', priority: 1 },
    { url: new URL('/proizvodi', baseUrl).href, changeFrequency: 'daily', priority: 0.9 },
    ...products.map((product) => ({
      url: new URL(`/proizvodi/${encodeURIComponent(product.slug)}`, baseUrl).href,
